...
from main import setup_logging, MeltomeAtlasHandler
...
def main():
    ...
    LOG_FILE = f'meltome_example_{timestamp_str}.log'
    LOG_PATH = os.path.join('', LOG_FILE)
    setup_logging(LOG_PATH, LOG_LEVEL)
    
    # Set output directory, default './results_meltome_$timestamp/'
    output_path = ''
    result_file = f'results_meltome_example_{timestamp_str}'
    output_path = os.path.join(output_path, result_file)
    
    logging.info(f"Log level: {logging.getLevelName(LOG_LEVEL)}")
    
    # Load & Process example dataset
    data_handler = MeltomeAtlasHandler(os.path.join('example', 'meltome_example.json'), output_path, LOG_LEVEL)
    data_handler.process_parallel(num_chunks=10)
    
    # Load generated results and compare them to expected output
    result_df = pd.read_csv(os.path.join(output_path, 'curve_fit.csv'))
    expected_df = pd.read_csv(os.path.join('example', 'expected_output', 'meltome_curve_fit.csv'))
    ...
    (result_df.tm_pred - expected_df.tm_pred).to_csv(os.path.join(output_path, 'tm_prediction_diff.csv'), index=False)
